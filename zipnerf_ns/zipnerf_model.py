...
class ZipNerfModelwarpper(Model):
    config: ZipNerfModelConfig

    def populate_modules(self):
        """Set the fields and modules."""
        super().populate_modules()
        print(self.config.dpcpp_backend)
        # config = Config()  # TODO: load config in .gin file and check the correction of save and load
        # config.PropMLP.disable_density_normals = True
        # config.PropMLP.disable_rgb = True
        # config.PropMLP.grid_level_dim = 1
        config=ZipNerfModelConfig()
        set_kwargs(config,vars(self.config))
        print(config)
        self.zipnerf = zipnerf(config=config)
        # self.zipnerf.nerf_mlp.disable_density_normals = True
        # self.zipnerf.prop_mlp_0.disable_density_normals = True
        # self.zipnerf.prop_mlp_1.disable_density_normals = True
        # self.zipnerf.prop_mlp_0.disable_rgb = True
        # self.zipnerf.prop_mlp_1.disable_rgb = True
        # self.zipnerf.prop_mlp_0.grid_level_dim = 1
        # self.zipnerf.prop_mlp_1.grid_level_dim = 1
        # Metrics
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        # TODO: deal with the difference between proposal_weights_anneal_max_num_iters and max_iter（especially for near anneal
        train_frac = np.clip((self.step - 1) / (25000-1), 0, 1)
        
        batch = self.construct_batch_from_raybundle(ray_bundle)
        renderings, ray_history = self.zipnerf(
                rand=True if self.training else False,
                batch=batch,
                train_frac=train_frac,
                compute_extras=self.config.compute_extras,
                zero_glo=False if self.training else True)
        outputs={}
